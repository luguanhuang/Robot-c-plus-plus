...
namespace future
{
    ...
    const string close_picture = "./res/e1.ico";
    const string log_picture = "./res/future.ico";
    class common
    {
    public:
        common(){};
        ...
        {
            // server_path = get_env_path();
            //QString conf_path = server_path + "/config/config.ini";
			
			QString conf_path = QCoreApplication::applicationDirPath() + "/config.ini";
        ...
        static void set_config_value(string key, string value)
        {
            try {
                ...
                //QString conf_path = server_path + "/config/config.ini";
				QString conf_path = QCoreApplication::applicationDirPath() + "/";
                //QString conf_path = QObject::tr("./config/config.ini");
                if (!QFile::exists(conf_path)) {
                    return;
                }
		...
		static QString getXorEncryptDecrypt1(const QString &str,
			const char &key = 17)
		{
			QString result;
