...
namespace future
{
    ...
    void future_platform::buy_order()
    {
        ...
        if (it != mapAccount.end())
        {
            if (it.value() == "trader_info")
            {
                ret = trader_server::get_instance()->manual_order_create(account, product, price, num, TAPI_SIDE_BUY);
            }
            else
            {
                ret = tradesvr1->manual_order_create(account, product, price, num, TAPI_SIDE_BUY);
            }
        }
    ...
    void future_platform::sell_order()
    {
        ...
        if (it != mapAccount.end())
        {
            if (it.value() == "trader_info")
            {
                ret = trader_server::get_instance()->manual_order_create(account, product, price, num, TAPI_SIDE_SELL);
            }
            else
            {
                ret = tradesvr1->manual_order_create(account, product, price, num, TAPI_SIDE_SELL);
            }
        }
    ...
    void future_platform::manual_create_mm_order()
    {
        ...
        if (it != mapAccount.end())
        {
            if (it.value() == "trader_info")
            {
                ...
                ret = trader_server::get_instance()->create_mm_order(
        ...
        {
            QMessageBox::information(w, QString().fromLocal8Bit("买入"), QString().fromLocal8Bit("买入挂单成功"));
            //HOLD 3s before cancel mm order
            //if (it.value() == "trader_info")
            //{
            //    //ret = trader_server::get_instance()->order_create(account, product, price, num, TAPI_SIDE_BUY);
            //    Sleep(3000);
    ...
    void future_platform::manual_cancel_mm_order()
    {
        ...
        char* buyorderno;
        ba = hkex_mm_buy_orderno_edit_->text().toLatin1(); // must
        buyorderno = ba.data();

        char* sellorderno;
        ba = hkex_mm_sell_orderno_edit_->text().toLatin1(); // must
        sellorderno = ba.data();
        ...
        if (it != mapAccount.end())
        {
            ...
            {
                //ret = trader_server::get_instance()->order_create(account, product, price, num, TAPI_SIDE_BUY);
                ret = trader_server::get_instance()->cancel_mm_order(
                    sell_select_check_->currentText().toLatin1().data(),
                    hkex_mm_buy_orderno_edit_->text().toLatin1().data(),
                    hkex_mm_sell_orderno_edit_->text().toLatin1().data());
            }
            else
            {
                //ret = tradesvr1->order_create(account, product, price, num, TAPI_SIDE_BUY);
                ret = tradesvr1->cancel_mm_order(
                    sell_select_check_->currentText().toLatin1().data(),
                    hkex_mm_buy_orderno_edit_->text().toLatin1().data(),
                    hkex_mm_sell_orderno_edit_->text().toLatin1().data());
            }
        ...
        if (ret)
        {
            QMessageBox::information(w, QString().fromLocal8Bit("买入"), QString().fromLocal8Bit("撤做市单失败"));
        }
        else
        {
            QMessageBox::information(w, QString().fromLocal8Bit("买入"), QString().fromLocal8Bit("撤做市单成功"));
            //if (it.value() == "trader_info")
            //{
            //    //ret = trader_server::get_instance()->order_create(account, product, price, num, TAPI_SIDE_BUY);
            //    Sleep(3000);
    ...
    int future_platform::check_select(const QString& text)
    {
        ...
        {   
                ...
                sell_gLayout->addWidget(buy, 4, 1);
                connect(buy, SIGNAL(clicked()), this, SLOT(manual_create_mm_order()));

                QLabel* buyorderno_label = new QLabel("BUYORERNO");
                hkex_mm_buy_orderno_edit_ = new QLineEdit();
                sell_gLayout->addWidget(buyorderno_label, 5, 1);
                sell_gLayout->addWidget(hkex_mm_buy_orderno_edit_, 5, 2);

                QLabel* sellorderno_label = new QLabel("SELLORERNO");
                hkex_mm_sell_orderno_edit_ = new QLineEdit();
                sell_gLayout->addWidget(sellorderno_label, 5, 3);
                sell_gLayout->addWidget(hkex_mm_sell_orderno_edit_, 5, 4);

                QPushButton* cancel_mm = new QPushButton("CANCEL MM ORDER");
                sell_gLayout->addWidget(cancel_mm, 6, 1);
                connect(cancel_mm, SIGNAL(clicked()), this, SLOT(manual_cancel_mm_order()));

                QLabel* select_ = new QLabel("选项");
                sell_type_check1_ = new QComboBox();
                sell_gLayout->addWidget(select_, 7, 1);
                sell_gLayout->addWidget(sell_type_check1_, 7, 2);
        ...
        //    //firstcreate = 1;
        //    
        //    w_cancel_order = new QWidget;
        //    QGridLayout CancelMMLayout;
        //    QLabel* aselect_label = new QLabel("账号");
        //    sell_select_check_ = new QComboBox();

        //    CancelMMLayout.addWidget(aselect_label, 0, 1);
        //    CancelMMLayout.addWidget(sell_select_check_, 0, 2);
        //    QStringList strList;
        //    for (int i = 0; i < m_vec_account.size(); i++)
        //    {
        //        strList << m_vec_account[i];
        ...
        //    QLabel* buyorderno_label = new QLabel("BUYORERNO");
        //    hkex_mm_buy_orderno_edit_ = new QLineEdit();
        //    CancelMMLayout.addWidget(buyorderno_label, 1, 1);
        //    CancelMMLayout.addWidget(hkex_mm_buy_orderno_edit_, 1, 2);

        //    QLabel* sellorderno_label = new QLabel("SELLORERNO");
        //    hkex_mm_sell_orderno_edit_ = new QLineEdit();
        //    CancelMMLayout.addWidget(sellorderno_label, 1, 3);
        //    CancelMMLayout.addWidget(hkex_mm_sell_orderno_edit_, 1, 4);
        ...
        //    QLabel* select_ = new QLabel("选项");
        //    sell_type_check1_ = new QComboBox();
        //    CancelMMLayout.addWidget(select_, 2, 1);
        //    CancelMMLayout.addWidget(sell_type_check1_, 2, 2);
        //    QStringList strList1;
        //    strList1 << "撤庄家单" << "庄家订单" << "普通交易" ;
        //    sell_type_check1_->addItems(strList1);

        //    QPushButton* buy = new QPushButton("发出");
        //    CancelMMLayout.addWidget(buy, 3, 1);
        ...
        //    w_cancel_order->setLayout(&CancelMMLayout);
        //    w_cancel_order->setWindowTitle("交易指示");
        //    connect(sell_type_check1_, SIGNAL(currentIndexChanged(const QString)), this, SLOT(check_select(const QString)));
        //    w_cancel_order->setAttribute(Qt::WA_QuitOnClose, true);
        //    w->setAttribute(Qt::WA_QuitOnClose, false);
        //    w1->setAttribute(Qt::WA_QuitOnClose, false);
        //    w_cancel_order->show();
        //    w->hide();
...
#define ONE 1
    ...
    void future_platform::addbuytrategy()
    {
       ...
       // out << res;//写入
        ...
        md_server::get_instance()->get_md_spi()->AddStratery(
            ...
            hedginginstrument1->text().toLatin1().data(),
            hedginginstrument2->text().toLatin1().data(),
            str);
    ...
    float future_platform::getprevalue(QString name)
    {
        ...
        {
            if (strstr( name.toLatin1().data(), m_vecPre[i].name.c_str()) != NULL)
            {
    ...
    int future_platform::GetBidAndAskData(QString strData, double &bid, double &ask, double &combid, double &comask)
    {
        ...
        string stradjustment = ba.data();
        double FAIRVALUE = MID1 * MID2 + atof(stradjustment.c_str());
        if (md_server::get_instance()->get_md_spi()->vecStrategy[idx].m_contract2flg.empty())
            FAIRVALUE = MID1 + atof(stradjustment.c_str());
      //  double FAIRVALUE = MID1 * MID2 + atof(stradjustment.c_str());
        char buf[64];
        snprintf(buf, sizeof(buf), "%f", md_server::get_instance()->get_md_spi()->m_info.CommodityTickSize);
        double HKEXCIN2112;
        ...
        md_server::get_instance()->get_md_spi()->m_info.CommodityTickSize = getprevalue(md_server::get_instance()->get_md_spi()->vecStrategy[idx].m_productflg.c_str());

        //md_server::get_instance()->get_md_spi()->m_info.CommodityTickSize = 0.01;
        //if (md_server::get_instance()->get_md_spi()->vecStrategy[idx].m_contract2flg.empty())
           // md_server::get_instance()->get_md_spi()->m_info.CommodityTickSize = 0.0001;
        double dtmp = md_server::get_instance()->get_md_spi()->m_info.CommodityTickSize;
        while (1)
        {
            wei++;
        ...
        //QString log_str = QObject::tr("product:%1,precision: %2, ticksize: %3")
        //    .arg(md_server::get_instance()->get_md_spi()->vecStrategy[idx].m_productflg.c_str())
        //    .arg(wei)
        //    .arg(md_server::get_instance()->get_md_spi()->m_info.CommodityTickSize);
        //slot_write_log(log_str);
        //printf("%d\n", wei);

        if (BIDASKSPREAD1 < atof(strthreshold.c_str()) && (BIDASKSPREAD2 < atof(strthreshold1.c_str()) || strthreshold1.empty()))
        {
            HKEXCIN2112 = FAIRVALUE - atof(strbidspread.c_str()) * md_server::get_instance()->get_md_spi()->m_info.CommodityTickSize;
            HKEXCIN2113 = FAIRVALUE + atof(straskspread.c_str()) * md_server::get_instance()->get_md_spi()->m_info.CommodityTickSize;
        }
        ...
        bid = Rurd(HKEXCIN2112, wei-1);
        ask = Rurd(HKEXCIN2113, wei-1);
        /*QString log_str = QObject::tr("bid spread tick(%1) * ticksize(%2)=bid spread(%3), fairvalue: %4, bid: %5")
            .arg(atof(strbidspread.c_str()))
            .arg(md_server::get_instance()->get_md_spi()->m_info.CommodityTickSize)
            .arg(atof(strbidspread.c_str()) * md_server::get_instance()->get_md_spi()->m_info.CommodityTickSize)
            .arg(FAIRVALUE)
            .arg(HKEXCIN2112);
        slot_write_log(log_str);*/
    ...
    void future_platform::slotActionOpen()
    {
        ...
        string bidprice = item->text().toLatin1().data();

        item = staterytableWidget->item(staterytableWidget->currentRow(), 6);
        string askprice = item->text().toLatin1().data();
        ...
        if (trader_server::get_instance()->get_trader_spi()->userid == list[23])
        {
            if (list[1]=="0")
            {
                ret = trader_server::get_instance()->get_trader_spi()->order_create(list[23].toLatin1().data(), contract, atof(bidprice.c_str()), atoi(list[2].toLatin1().data()), TAPI_SIDE_BUY, idx);
            }
            
            else if (list[1] == "2")
            {
                ret = trader_server::get_instance()->get_trader_spi()->bi_order_create(list[23].toLatin1().data(), contract, atof(bidprice.c_str()), atof(askprice.c_str()), atoi(list[2].toLatin1().data()), idx);
            }
        ...
        {
            tradesvr1->get_trader_spi()->order_create(list[23].toLatin1().data(), contract, atof(bidprice.c_str()), atoi(list[2].toLatin1().data()), TAPI_SIDE_BUY, idx);
            
            QString log_str = QObject::tr("Order created Strategy idx=%1 has member stClientOrderNo=%2").arg(idx).
                arg(md_server::get_instance()->get_md_spi()->vecStrategy[idx].stClientOrderNo.c_str());
            slot_write_log(log_str);
        }
      ...
      //  box.exec();
       ...
       // box.exec();
        ...
        item = staterytableWidget->item(staterytableWidget->currentRow(), 0);
        md_server::get_instance()->get_md_spi()->vecStrategy[idx].m_isopen = true;
        QFont nullFont;
...
#if 1
    void future_platform::update_strategy_status_stopped(int idx)
    {
        QTableWidgetItem* item;
        item = staterytableWidget->item(idx, 0);
        QFont nullFont;
        nullFont.setBold(true);
        QBrush nullColor(Qt::red);
        item->setData(0, "stopped");
        item->setFont(nullFont);
        item->setBackground(nullColor);
        md_server::get_instance()->get_md_spi()->vecStrategy[idx].m_mapCreateOrder.clear();
        md_server::get_instance()->get_md_spi()->vecStrategy[idx].m_mapCancelOrder.clear();
        md_server::get_instance()->get_md_spi()->vecStrategy[idx].Buy_OrderNo = -1;
        md_server::get_instance()->get_md_spi()->vecStrategy[idx].Sell_OrderNo = -1;
    }
    
    void future_platform::slotActionStop()
    {
        ...
        QMap<QString, QString>::iterator it = md_server::get_instance()->get_md_spi()->vecStrategy[idx].m_mapCreateOrder.begin();
        if (it == md_server::get_instance()->get_md_spi()->vecStrategy[idx].m_mapCreateOrder.end())
        {
            return;
        ...
        if (trader_server::get_instance()->get_trader_spi()->userid == list[23])
        {
            ...
            string str = it.key().toLatin1().data();
            if (list[1] == "0") 
            { 
            ...
            { 
                trader_server::get_instance()->get_trader_spi()->bi_order_withdraw(it.key().toLatin1().data(), it++.key().toLatin1().data()); 
            }
        ...
        {
            md_server::get_instance()->get_md_spi()->vecStrategy[idx].m_cancelflg = true;
            if (list[1] == 0) { tradesvr1->get_trader_spi()->order_withdraw(it.key().toLatin1().data()); }
            else if (list[1] == 2) { tradesvr1->get_trader_spi()->bi_order_withdraw(it.key().toLatin1().data(), it++.key().toLatin1().data()); }
        }
      ...
      //  box.exec();
       ...
       // box.exec();
        ...
        item = staterytableWidget->item(staterytableWidget->currentRow(), 0);
        md_server::get_instance()->get_md_spi()->vecStrategy[idx].m_isopen = false;
        QFont nullFont;
    ...
    void future_platform::show_stratery()
    {
        ...
        for (int i = 0; i < string_list.size(); i++)
        {
            ...
            //if (md_server::get_instance()->get_md_spi()->vecStrategy.size() == 0)
            {
               md_server::get_instance()->get_md_spi()->AddStratery(
                ...
                list[14].toLatin1().data(),
                list[17].toLatin1().data(),
                    data.data());
                ...
                md_server::get_instance()->get_md_spi()->
                    req_sub_market_data(list[14].toLatin1().data());

                if (!list[17].isEmpty())
                {
                    md_server::get_instance()->get_md_spi()->
                        req_sub_market_data(list[17].toLatin1().data());
                }
            }
          ...
          //  Sleep(1000);
            ...
            nullFont.setBold(true);
            QBrush nullColor(Qt::red);
            QTableWidgetItem* item = new QTableWidgetItem("stopped");
            item->setFont(nullFont);
            item->setBackground(nullColor);
            staterytableWidget->setItem(line, col++, item);
            ...
            char buf[16];
            snprintf(buf, sizeof(buf), "%.4f", bid);
            

            staterytableWidget->setItem(line, col++, new QTableWidgetItem(buf));
            
            snprintf(buf, sizeof(buf), "%.4f", combid);
            staterytableWidget->setItem(line, col++, new QTableWidgetItem(buf));
            snprintf(buf, sizeof(buf), "%.4f", comask);
            staterytableWidget->setItem(line, col++, new QTableWidgetItem(buf));
            
            snprintf(buf, sizeof(buf), "%.4f", ask);
            staterytableWidget->setItem(line, col++, new QTableWidgetItem(buf));
       ...
       connect(stopaction, SIGNAL(triggered()), this, SLOT(slotActionStop()));
       stratery_widget_menu->addAction(stopaction);
    ...
    void future_platform::GetPreInfo()
    {
        ...
        while (!feof(fp))
        {
            ...
            QStringList list = str.split(" ");
            StPrecision pre;
            pre.name = list[0].toLatin1().data();
            pre.value = atof(list[1].toLatin1().data());
    ...
    void future_platform::ActionOpen(int idx)
    {
        double bid;
        double ask;
        double combid;
        double comask;
        GetBidAndAskData(md_server::get_instance()->get_md_spi()->vecStrategy[idx].m_linedata.c_str(), bid, ask, combid, comask);
      ...
      /*  item = staterytableWidget->item(row, 2);
        ...
        if (trader_server::get_instance()->get_trader_spi()->userid == list[23])
            if (list[1] == "0")
            {
                ret = trader_server::get_instance()->get_trader_spi()->order_create(list[23].toLatin1().data(), list[0].toLatin1().data(), bid, atoi(list[2].toLatin1().data()), TAPI_SIDE_BUY, idx);
            }
            else if (list[1] == "2")
            {
                ret = trader_server::get_instance()->get_trader_spi()->bi_order_create(list[23].toLatin1().data(), list[0].toLatin1().data(), bid, ask, atoi(list[2].toLatin1().data()), idx);
            }
        else if (tradesvr1->get_trader_spi()->userid == list[23])
                if (list[1] == "0")
                {
                    ret = tradesvr1->get_trader_spi()->order_create(list[23].toLatin1().data(), list[0].toLatin1().data(), bid, atoi(list[2].toLatin1().data()), TAPI_SIDE_BUY, idx);
                }
            else if (list[1] == "2")
                {ret = tradesvr1->get_trader_spi()->bi_order_create(list[23].toLatin1().data(), list[0].toLatin1().data(), bid, ask, atoi(list[2].toLatin1().data()), idx);}
    ...
    void future_platform::slot_update_stratery(int idx)
    {
        if (NULL != staterylist && true == staterylist->isVisible())
        {
            ...
            for (int i = 0; i < rowcount; i++)
            {
                ...
                if (item != NULL && item->text() == list[21])
                {
                    ...
                    int ret;
                    if (true == md_server::get_instance()->get_md_spi()->vecStrategy[idx].m_isopen)
                    {
                        if (trader_server::get_instance()->get_trader_spi()->userid == list[23])
                        {
                            QString log_str = QObject::tr("%1%2%3%4").arg("idx1111=").arg(idx).arg("cancelflg=").
                                arg(md_server::get_instance()->get_md_spi()->vecStrategy[idx].m_cancelflg);
                            slot_write_log(log_str);
                            QMap<QString, QString>::Iterator it = md_server::get_instance()->get_md_spi()->vecStrategy[idx].m_mapCancelOrder.begin();
                            if (it == md_server::get_instance()->get_md_spi()->vecStrategy[idx].m_mapCancelOrder.end())
                            {
                               
                                //ActionOpen(i);
                            }

                            it = md_server::get_instance()->get_md_spi()->vecStrategy[idx].m_mapCreateOrder.begin();
                            if (it != md_server::get_instance()->get_md_spi()->vecStrategy[idx].m_mapCreateOrder.end()
                                && false == md_server::get_instance()->get_md_spi()->vecStrategy[idx].m_cancelflg)
                            {
                                ...
                                {
                                    /*if (it.key().toLatin1().data() == md_server::get_instance()->get_md_spi()->vecStrategy[idx].Buy_OrderNo)
                                    {
                                        trader_server::get_instance()->get_trader_spi()->bi_order_withdraw(it.key().toLatin1().data(), it++.key().toLatin1().data());
                                    }
                                    else 
                                        trader_server::get_instance()->get_trader_spi()->bi_order_withdraw(it++.key().toLatin1().data(), it.key().toLatin1().data());*/
                                    trader_server::get_instance()->get_trader_spi()->bi_order_withdraw(md_server::get_instance()->get_md_spi()->vecStrategy[idx].Buy_OrderNo
                                        , md_server::get_instance()->get_md_spi()->vecStrategy[idx].Sell_OrderNo);
                                    break;
                                }
                            }
                            else
                            {
                                QString log_str = QObject::tr("%1%2%3%4").arg("idx2222=").arg(idx).arg("cancelflg=").
                                    arg(md_server::get_instance()->get_md_spi()->vecStrategy[idx].m_cancelflg);
                                slot_write_log(log_str);
                            }
                        ...
                        else if (tradesvr1->get_trader_spi()->userid == list[23])
                        {
                                ...
                                && false == md_server::get_instance()->get_md_spi()->vecStrategy[idx].m_cancelflg)
                            {
                                ...
                                else if (list[1] == "2")
                                {
                                    /*if (it.key().toLatin1().data() == md_server::get_instance()->get_md_spi()->vecStrategy[idx].Buy_OrderNo)
                                    {
                                        tradesvr1->get_trader_spi()->bi_order_withdraw(it.key().toLatin1().data(), it++.key().toLatin1().data());
                                    }
                                    else 
                                    {*/
                                        tradesvr1->get_trader_spi()->bi_order_withdraw(md_server::get_instance()->get_md_spi()->vecStrategy[idx].Buy_OrderNo, 
                                            md_server::get_instance()->get_md_spi()->vecStrategy[idx].Sell_OrderNo);
                                    /*}*/
                    ...
                    snprintf(buf, sizeof(buf), "%.4f", md_server::get_instance()->get_md_spi()->vecStrategy[idx].m_productinfo.QBidPrice[0]);
                    staterytableWidget->setItem(i, 4, new QTableWidgetItem(buf));
                    snprintf(buf, sizeof(buf), "%.4f", md_server::get_instance()->get_md_spi()->vecStrategy[idx].m_productinfo.QAskPrice[0]);
                    staterytableWidget->setItem(i, 5, new QTableWidgetItem(buf));
                    staterytableWidget->viewport()->update();
                    break;
                }
