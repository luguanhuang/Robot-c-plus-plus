...
namespace future {
    ...
    void applog::log(level elevel, const std::string& info, const char *msg)
    {
        ...
        if(1) {
            ...
            if(enable_file_log && elevel >= file_log_level) {
                std::string file = "交易日志";
                {
                    ...
                    log_file += "/";
                    log_file += file;
                    std::ofstream tt2(log_file.c_str(), std::ios::app| std::ios::binary);
                    FILE* fpWrite = fopen(log_file.c_str(), "a+");
                    string str;
                    str = get_time_str();
                    str += "/";
                    str+= LOG_LEVEL_STR[elevel];
                    str +=  ":    ";
                    str +=  msg;
                    tt2 << "    ";
                    unsigned int pos;
